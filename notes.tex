...
\begin{definition}[Mass production]
    The net mass rate of production of a species per unit volume is found with:

    \begin{equation}
        \dot{w}_i = \mathcal{M}_i \sum_{r=1}^\text{NR} \bp{\frac{dX_i}{dt}}_r = \sum_{r=1}^\text{NR} \bp{\frac{d \rho_i}{d t}}_r
    \end{equation}

    of 

    \begin{equation}
        \dot{w}_i = \mathcal{M}_i \sum_{r=1}^\text{NR} (\beta_{i,r} - \alpha_{i,r})(R_{f,r} - R_{b,r})
    \end{equation}

    Where 
    \begin{align}
         R_{f,r} &= k_{f,r} \prod_{j=1}^\text{NJ} (\gamma_j \rho)^{\alpha_{j,r}}     
         \\ R_{b,r} &= k_{b,r} \prod_{j=1}^\text{NJ} (\gamma_j \rho)^{\beta_{j,r}}     
    \end{align}
