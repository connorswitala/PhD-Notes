...
\begin{multline}
    \pd{<\tau_s>}{U_j} = \sum_{r=1}^\text{NS} X_r \bb{\pd{}{U_j} \bp{\sum_{r=1}^\text{NS} \frac{X_r}{\tau_{sr}}}^{-1} }
    \\ = - \bb{\sum_{r=1}^\text{NS} X_r} \bb{\sum_{r=1}^\text{NS} \frac{X_r}{\tau_{sr}}}^{-2} \bcb{\sum_{r=1}^\text{NS} \bb{ \bp{\pd{X_r}{U_j} } \frac{1}{\tau_{sr}} + X_r \bp{\pd{}{U_j} \frac{1}{\tau_{sr}}}}}
\end{multline}

The first term inside the curly brace expands to:
...
\begin{eqnarray}
    \boxed{\pd{X_r}{\rho_j} = \frac{1}{\sum_{i=1}^\text{NS} \rho_i / \mathcal{M}_i} \bb{\frac{\delta_{jr}}{\mathcal{M}_r} - \frac{X_r}{\mathcal{M}_j}}}
\end{eqnarray}

And all other derivatives WRT conserved variables are 0. $\delta_{jr}$ is the Kronecker delta. Then $\rho_j$ = $\rho_r$, it is 1, otherwise it is 0. The second term expands to the following without Park's correction:

\begin{align}
    X_r \bp{\pd{}{U_j} \frac{1}{\tau_{sr}}} &= - X_r \tau_{sr}^{-2} \pd{\tau_{sr}}{U_j}
    \\ &= -X_r \tau_{sr}^{-2} \bb{-\tau_{sr} \bp{\frac{101325}{p} \pd{p}{U_j} + \frac{1}{3} \pd{T}{U_j} T^{-4/3} A_{sr}}}
    \\ &=  \frac{X_r}{\tau_{sr}} \bb{\frac{101325}{p} \pd{p}{U_j} + \frac{1}{3} \pd{T}{U_j} T^{-4/3} A_{sr}}
\end{align}
...
\begin{equation}
    \pd{\tau_{sr}^\text{MW}}{U_j} = -\tau_{sr}^\text{MW} \bb{\frac{101325}{p} \pd{p}{U_j} + \frac{1}{3} \pd{T}{U_j} T^{-4/3} A_{sr}}
\end{equation}

The harder derivative is:

\begin{equation}
    \pd{\tau_{sr}^\text{P}}{U_j} = \pd{}{U_j} \bb{\frac{1}{\bar{c} \sigma n}} = -(\bar{c} \sigma n)^{-2} \bcb{\pd{\bar{c}}{U_j} \sigma n + \bar{c} \bb{\pd{\sigma}{U_j} + \sigma \pd{n}{U_j}}}
\end{equation}
