...
\begin{equation}
    \frac{d [X_i]}{dt} = (\nu_i'' - \nu_i') \underbracket{\sum_{i=1}^\text{NS} \epsilon_i [M_i]}_{E_\text{eff}} \underbracket{\left \{ \gamma(T_f) [X_{s,f}]^{\nu_s'} - \frac{\gamma(T_b)}{K_c(T_b)} [X_{s,f}]^{\nu_s''} \right \}}_{L_r}
\end{equation}
...
\begin{equation}
    \pd{E_\text{eff}}{U_j} = 
    \begin{cases}
        E_\text{eff} + \frac{\epsilon_j}{\mathcal{M}_j} (1 - \rho_j), \hspace{5mm} \text{if $U_j$ = $\rho_j$}
