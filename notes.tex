...
The conserved variable vector convective flux vector for the stagnation line solver is:
...
Some very useful identities for the Jacobian of the convective fluxes are:

\begin{equation*}
    \rho E = \underbracket{\sum_s \rho_s e_s^\text{tr-rot}}_{\rho e^\text{tr}} + \underbracket{\sum_s \rho_s e_s^\text{vib}}_{\rho e^\text{ve}} + \frac{1}{2} \rho u_r^2 + \sum_s \rho_s h_s^f
\end{equation*}

\begin{equation*}
    \rho E = \bb{\frac{\sum_s \rho_s c_{v,s}}{\sum_s \rho_s R_s}} p + \sum_s^d \rho_s e_s^{ve} + \sum_s \rho_s u_r^2 + \sum_s \rho_s h_s^f
\end{equation*}

\begin{equation*}
    p = \rho R_\text{mix} T_\text{tr} = \sum_s \rho_s R_s T_\text{tr}
\end{equation*}

\begin{equation*}
    \rho e^{ve} = \sum_s \rho_s e_s^\text{vib}
\end{equation*}
...
Per Scoggins, the most useful derivatives are the ones of pressure and total energy. His notation for the pressure derivatives is:

\begin{equation*}
    \pd{p}{U} = \begin{bmatrix}
        \Phi_j \\
        -\beta^1 u_r \\
        0 \\
        \beta^1 \\
...
Let us begin with $\phi_j$ using the above equation of state for a mixture of ideal gases. In order to continue, we must find the derivatives of the translational temperature with respect to the conserved variables. This is done by using the definition of the translational energy:

\begin{equation*}
    \rho e^\text{tr} = \sum_s \rho_s e_s^\text{tr-rot}
\end{equation*}

as well as using the total energy equation and solving for $E_\text{tr}$:

\begin{equation*}
    \rho e^\text{tr} = \rho E - \rho e^\text{ve} - \frac{1}{2} \rho u_r^2 - \sum_s \rho_s h_s^f
\end{equation*}

We can now implcitly solve for the translational temperature derivatives:

\begin{equation*}
    \pd{(\rho e^\text{tr})}{U_j} = \pd{(\rho E)}{U_j} - \pd{(\rho e^\text{ve})}{U_j} - \pd{}{U_j} \bb{\frac{1}{2} \rho u_r^2} - \pd{}{U_j} \bb{\sum_s \rho_s h_s^f}
\end{equation*}

This gives:

\begin{align*}
    \pd{(\rho e^\text{tr})}{\rho_j} &= \frac{1}{2} u_r^2 - h_j^f
    \\ \pd{(\rho e^\text{tr})}{\rho u_r} &= -u_r
    \\ \pd{(\rho e^\text{tr})}{\rho u_\theta} &= 0
    \\ \pd{(\rho e^\text{tr})}{\rho E} &= 1
    \\ \pd{(\rho e^\text{tr})}{\rho e^{ve}} &= -1
\end{align*}

Then we go to the first equation:

\begin{equation*}
    \pd{(\rho e^\text{tr})}{U_j} = \pd{}{U_j} \bb{\sum_s \rho_s e_s^\text{tr}} = \sum_s \bb{\pd{}{U_j} \rho_s e_s^\text{tr}} = \sum_s \bb{ \pd{\rho_s}{U_j} e_s^\text{tr} + \rho_s \pd{e_s^\text{tr}}{T_\text{tr}} \pd{T_\text{tr}}{U_j} }
\end{equation*}

Which becomes:

\begin{equation*}
    \pd{(\rho e^\text{tr})}{U_j} = \underbracket{\delta_{js} e_j^\text{tr}}_{\text{if $U_j$ is $\rho_j$}} + \rho c_{v,\text{mix}}^\text{tr} \pd{T_\text{tr}}{U_j} 
\end{equation*}


Now we can solve for the derivative of the translational temperature by setting the two equations equal to each other:

\vspace{5mm}
$\partial / \partial \rho_j$:

\begin{equation*}
    \frac{1}{2} u_r^2 - h_j^f = e_j^\text{tr} + \rho c_{v,\text{mix}}^\text{tr} \pd{T_\text{tr}}{\rho_j}
\end{equation*}

\begin{equation*}
    \boxed{\pd{T_\text{tr}}{\rho_j} = \frac{\frac{1}{2} u_r^2 - h_j^f - e_j^\text{tr}}{\rho c_{v,\text{mix}}^\text{tr}}}
\end{equation*}

\vspace{5mm}
$\partial / \partial \rho u_r$:

\begin{equation*}
    -u_r = \rho c_{v,\text{mix}}^\text{tr} \pd{T_\text{tr}}{\rho u_r}
\end{equation*}

\begin{equation*}
    \boxed{\pd{T_\text{tr}}{\rho u_r} = -\frac{u_r}{\rho c_{v,\text{mix}}^\text{tr}}}
\end{equation*}

\vspace{5mm}
$\partial / \partial \rho u_\theta$:

\begin{equation*}
    0 = \rho c_{v,\text{mix}}^\text{tr} \pd{T_\text{tr}}{\rho u_\theta}
\end{equation*}

\begin{equation*}
    \boxed{\pd{T_\text{tr}}{\rho u_\theta} = 0}
\end{equation*}

\vspace{5mm}
$\partial / \partial \rho e^\text{tr}$:

\begin{equation*}
    1 = \rho c_{v,\text{mix}}^\text{tr} \pd{T_\text{tr}}{\rho E}
\end{equation*}

\begin{equation*}
    \boxed{\pd{T_\text{tr}}{\rho E} = \frac{1}{\rho c_{v,\text{mix}}^\text{tr}}}
\end{equation*}

\vspace{5mm}
$\partial / \partial \rho e^{ve}$:
\begin{equation*}
    -1 = \rho c_{v,\text{mix}}^\text{tr} \pd{T_\text{tr}}{\rho e^{ve}}
\end{equation*}

\begin{equation*}
    \boxed{\pd{T_\text{tr}}{\rho e^{ve}} = -\frac{1}{\rho c_{v,\text{mix}}^\text{tr}}}
\end{equation*}

Therefore, the full derivatives of pressure WRT the conserved variables are:

\begin{equation*}
    \boxed{\Phi_j = \pd{p}{\rho_j} = R_j T_\text{tr} + \sum_s \rho_s R_s \pd{T_\text{tr}}{\rho_j} = R_j T_\text{tr} + \sum_s \rho_s R_s \frac{-\frac{1}{2} u_r^2 - h_j^f - e_j^\text{tr}}{\rho c_{v,\text{mix}}^\text{tr}}}
\end{equation*}

\begin{equation*}
    \boxed{\beta^1 = \frac{R_\text{mix}}{c_{v,\text{mix}}^\text{tr}} }
\end{equation*}
...
\begin{equation}
    \boxed{\beta^n - \beta^1 = -\frac{R_\text{mix}}{c_{v,\text{mix}}^\text{tr}}}
\end{equation}

\subsection{Energy derivatives}

We utilize the following equation for the energy derivatives (putting them into functions of primitive variables):
...
\begin{align*}
    \pd{(\rho E)}{\rho_j} &= \bb{c_{v,j}^\text{tr-rot} - \frac{c_{v, \text{mix}}^\text{tr-rot}}{R_\text{mix}} R_j} T_\text{tr} + \frac{u_r^2}{2} + e_j^{ve} + h_j^f
    \\ \pd{(\rho E)}{u_r} &= \rho u_R
    \\ \pd{(\rho E)}{u_\theta} &= 0
    \\ \pd{(\rho E)}{p} &= \frac{c_{v,\text{mix}}^\text{tr-rot}}{R_\text{mix}}
    \\ \pd{(\rho E)}{e_v} &= \sum_d^\text{ND} \rho_d
\end{align*}

Where the finally entry is the sum of the densities of polyatomic molecules in the gas mixture.
