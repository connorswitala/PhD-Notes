...
The conserved variable vector, primitive variable vector, and convective flux vector for the stagnation line solver are:
...
In order to find the flux Jacobian, we need to compute a series of Jacobians. The first is the Jacobian of $\boldsymbol{U}$ with respect to $\boldsymbol{V}$. This is done by putting the conserved variables in terms of the primitive variables and then taking the derivatives. Then for the Jacobians of $\boldsymbol{V}$ with respect to $\boldsymbol{U}$, we do the same. By denoting $\rho_s$ as either $U_s$ or $V_s$, $\rho u_r$ as $U_2$, $u_r$ as $V_2$, etc., we can write the vectors as:
...
\begin{equation*}
    ...
    \end{bmatrix}
    \hspace{5mm}
    \boldsymbol{\boldsymbol{F^c}(\boldsymbol{V})} = \begin{bmatrix}
        V_s V_2 \\
        \sum_s V_s V_2^2 + V_5 \\
        \sum_s V_s V_2 V_3\\
        \sum_s V_s V_2 V_4 \\
        [\rho E(\boldsymbol{V}) + V_5]V_2
    \end{bmatrix}   
\end{equation*}
...
\begin{equation*}
    p = \rho R_\text{mix} T_\text{tr} = \frac{R_\text{mix}}{c_{v, \text{mix}}^\text{tr}} \bb{\rho e^\text{tr}} = \frac{R_\text{mix}}{c_{v, \text{mix}}^\text{tr}} \bb{\rho E - \frac{\rho}{2} u_r^2 - \rho e_v - \sum_s \rho_s h_s^f}
\end{equation*}

In conserved variable form, this is:

\begin{equation*}
    p(\boldsymbol{U}) = \frac{\sum U_s R_s}{\sum U_s c_{v,s}^\text{tr}} \bb{U_5 - \frac{1}{2} \frac{1}{\sum_s U_s} U_2^2 - U_4 - \sum_s U_s h_s^\circ}
\end{equation*}

For internal energy, we can rearrange the above equation to get:

\begin{equation*}
    \rho E = \frac{c_{v, \text{mix}}^\text{tr}}{R_\text{mix}} p + \frac{1}{2} \rho u_r^2 + \rho e_v + \sum_s \rho_s h_s^f
\end{equation*}

In primitive variable form, this is:

\begin{equation*}
    \rho E(\boldsymbol{V}) = \frac{\sum_s V_s c_{v,s}}{\sum_s V_s R_s} V_5 + \frac{1}{2} \sum_s V_s \cdot V_2^2 + \sum_s V_s \cdot V_4 + \sum_s V_s h_s^\circ
\end{equation*}
...
\begin{equation*}
    \pd{\boldsymbol{U}}{\boldsymbol{V}} = \begin{bmatrix}
        ...
        \pd{\rho E}{\rho_s} & \dots & \pd{\rho E}{\rho_s} & \pd{\rho E}{u_r} & \pd{\rho E}{u_\theta} & \pd{\rho E}{e_v} & \pd{\rho E}{p}
    \end{bmatrix}
\end{equation*}
...
\begin{equation*}
    \pd{\boldsymbol{V}}{\boldsymbol{U}} = \begin{bmatrix}
        ...
        \pd{p}{\rho_s} & \dots & \pd{p}{\rho_s} & \pd{p}{(\rho u_r)} & \pd{p}{(\rho u_\theta)} & \pd{p}{(\rho e_v)} & \pd{p}{(\rho E)}
    \end{bmatrix}
\end{equation*}

\begin{equation*}
    \pd{\boldsymbol{F^c}}{\boldsymbol{V}} = \begin{bmatrix}
        u_r & 0 & 0 & \rho_s & 0 & 0 & 0 \\
        0 & \ddots & 0 & \vdots & 0 & 0 & 0 \\
        0 & 0 & u_r & \rho_s & 0 & 0 & 0 \\
        u_r^2 & \dots & u_r^2 & 2 \rho u_r & 0 & 0 & 1 \\
        u_r u_\theta & \dots & u_r u_\theta & \rho u_\theta & \rho u_r & 0 & 0 \\
        u_r e_{v} & \dots & u_r e_{v} & \rho e_{v} & 0 & \rho u_r & 0 \\
        \pd{\rho E}{\rho_s} u_r & \dots & \pd{\rho E}{\rho_s} u_r & \Phi & \pd{\rho E}{u_\theta} u_r & \pd{\rho E}{e_v} u_r & \Theta
    \end{bmatrix}
\end{equation*}

Where we have the following derivatives for pressure and energy as well as the following definitions for $\Phi$ and $\Theta$:
...
The matrix product of $\partial V / \partial U$ and $\partial F^c / \partial V$ gives the flux Jacobian that needs to be diagonlized for the upwinding:

\begin{equation*}
    \pd{\boldsymbol{V}}{\boldsymbol{U}} \pd{\boldsymbol{F^c}}{\boldsymbol{V}} = \begin{bmatrix}
        u_r & 0 & 0 & \rho_s & 0 & 0 & 0 \\
        0 & \ddots & 0 & \vdots & 0 & 0 & 0 \\
        0 & 0 & u_r & \rho_s & 0 & 0 & 0 \\
...
\begin{equation*}
    \pd{U}{V} R \Lambda = \begin{bmatrix}
        \lambda_1 & 0 & 0 & 0 & 0 & \rho_s \lambda_2 & \rho_s \lambda_3 \\
        0 & \ddots & 0 & 0 & 0 & \vdots & \vdots \\        
        0 & \dots & \lambda_1 & 0 & 0 & \rho_s \lambda_2 & \rho_s \lambda_3 \\
        u_r \lambda_1 & \dots & u_r \lambda_1 & 0 & 0 & \rho \lambda_2^2 & \rho \lambda_3^2  \\
        u_\theta \lambda_1 & \dots & u_\theta \lambda_1 & \rho \lambda_1 & 0 & \rho u_\theta \lambda_2 & \rho u_\theta \lambda_3 \\
        e_v \lambda_1 & \dots & e_v \lambda_1 & 0 & \rho \lambda_1 & \rho e_v \lambda_2 & \rho e_v \lambda_3 \\
        E_{\rho_s} \lambda_1 & \dots & E_{\rho_s} \lambda_1 & \rho a \frac{\bar{c}_v}{\bar{R}} \lambda_1 & \rho \lambda_1 & \Psi^- \lambda_2 & \Psi^+ \lambda_3 
    \end{bmatrix}
\end{equation*}
