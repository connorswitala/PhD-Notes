...
\begin{definition}[Efficiencies]
    ...
    For a third body reaction, the change in the concentration of $N_2$ is then:

    \begin{align*}
        \frac{d[N_2]}{dt} &= (0 - 1)\left\{ \epsilon \gamma [N_2][M] - \frac{\epsilon \gamma}{K_{eq}}[N]^2[M] \right\} 
        \\ &= -1 \cdot \epsilon [M] \left\{ \gamma [N_2] - \frac{\gamma}{K_{eq}}[N]^2 \right\}
        \\ &= -1 \cdot \epsilon [M] R
    \end{align*}
    ...
    \begin{align*}
        \frac{d[N_2]}{dt} &= \sum_{i} \epsilon_i [M_i] R
    \end{align*}
...
\subsection{Landau-Teller model}
...
\subsubsection{Source term Jacobians}

The general equation for the trans-vib relaxation source term Jacobian entries is:

\begin{equation}
    \pd{}{U_i} Q_{t-v,s}^{LT} = \delta_{si} \frac{e_{vs}^* - e_{vs}}{<\tau_s>} + \frac{\rho_s}{<\tau_s>} \pd{}{U_i}(e_{vs}^* - e_{vs}) - \rho_s \frac{e_{vs}^* - e_{vs}}{<\tau_s>^2} \pd{}{U_i} <\tau_s>
\end{equation}

Given the following derivatives:
...
\begin{align}
    ...
    \\ \pd{T_v}{(\rho u)} = 0, \hspace{10mm} \pd{T_v}{(\rho v)} &= 0, \hspace{10mm} \pd{T_v}{(\rho w)} = 0
    \\ \pd{T_v}{E_v} &= 1 / \sum_{s=1}^\text{NS} \bb{\rho_s \bp{\pd{e_{vs}}{T_v}}}
    \\ \pd{T_v}{E} &= 0
\end{align}


The individual Jacobian entries are then:
