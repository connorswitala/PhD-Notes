...
One mole of a substance contains Avogado's number of particles. The molecular weight of a substance is the mass of one mole of that substance. In this text it is denoted as $\mathcal{M}$ and is typically given in units of kg / kg-mole or g / gm-mole. For example, the molecular weight of $O_2$ is 32 kg / kg-mole, and the molecular weight of $N_2$ is 28 kg / kg-mole.
\label{def:MW}
\end{definition}
...
\begin{equation*}
    \boldsymbol{L} = \tilde{L} \pd{V}{U} = \begin{bmatrix}
...
\begin{equation*}
    \boldsymbol{R} = \pd{U}{V} \tilde{R} = \begin{bmatrix}
        1 & 0 & 0 & 0 & 0 & \rho_s & \rho_s \\
        0 & \ddots & 0 & 0 & 0 & \vdots & \vdots \\
        0 & \dots & 1 & 0 & 0 & \rho_s & \rho_s \\
...
\begin{multline}
    \frac{\delta \boldsymbol{U}_i^{n+1}}{\Delta t} \Delta r_i = \bb{\boldsymbol{F_c}_{,i-\frac{1}{2}}^n + \boldsymbol{A}_{i-\frac{1}{2}}^+ \delta \boldsymbol{U}_{i-1}^{n+1} + \boldsymbol{A}_{i-\frac{1}{2}}^- \delta \boldsymbol{U}_{i}^{n+1}} 
    \\ - \bb{\boldsymbol{F_c}_{,i+\frac{1}{2}}^n + \boldsymbol{A}_{i+\frac{1}{2}}^+ \delta \boldsymbol{U}_{i}^{n+1} + \boldsymbol{A}_{i+\frac{1}{2}}^- \delta \boldsymbol{U}_{i+1}^{n+1}} + \boldsymbol{S_c}_{,i}^n + \boldsymbol{J}_c^S \delta \boldsymbol{U}_i^{n+1}
\end{multline}

Solving this equation for the update $\delta U$ gives:

\begin{multline}
    \textcolor{defred}{\bb{\boldsymbol{A}_{i + \frac{1}{2}}^-}} \delta \boldsymbol{U}_{i+1}^{n+1} + \textcolor{defblue}{\bb{\frac{\Delta r_i}{\Delta t} \boldsymbol{I} + \boldsymbol{A}_{i+\frac{1}{2}}^+ - \boldsymbol{A}_{i-\frac{1}{2}}^- - \boldsymbol{J}_c^S}} \delta \boldsymbol{U}_i^{n+1} + \textcolor{deforange}{\bb{-\boldsymbol{A}_{i-\frac{1}{2}}^+}}\delta \boldsymbol{U}_{i-1}^{n+1} = \textcolor{defgreen}{\boldsymbol{F_c}_{,i-\frac{1}{2}}^n - \boldsymbol{F_c}_{,i+\frac{1}{2}}^n + \boldsymbol{S_c}_{,i}^n}
\end{multline}

Which gives a block tridiagonal system of equations to be solved for:

\begin{equation}
    \textcolor{defred}{\boldsymbol{A}} \boldsymbol{U}_{i+1}^{n+1} + \textcolor{defblue}{\boldsymbol{B}} \boldsymbol{U}_{i}^{n+1} + \textcolor{deforange}{\boldsymbol{C}} \boldsymbol{U}_{i-1}^{n+1} = \textcolor{defgreen}{\boldsymbol{D}}
\end{equation}
