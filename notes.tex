...
\begin{definition}[Reactions]
    ...
    \begin{eqnarray}
        \sum_{i=1}^\text{NJ} \alpha_{i,r} X_i \leftrightarrows \sum_{i=1}^\text{NJ} \beta_{i,r} X_i
    \end{eqnarray}

    Where NJ is is the sum of NS with the number of catalytic bodies. $\alpha$ and $\beta$ are the stroichiometric coefficients for reactants and products respectively. $k_{f,r}$ foverns the forward reaction (right arrow), and $k_{b,r}$ governs the backward reaction (left arrow). Catalytic bodies are species that do no undergo chemical change. 
    
\end{definition}

\begin{definition}[Mass production]
    The net mass rate of production of a species per unit volume has a number of different expressions. Consider the reaction:
    ...
    Here, M is an arbitrary collision partner. Concentration of species $j$ is the moles of $j$ per unit volume. $[O_2]$ is the concentration of $O_2$. The time rate of change of the concentration of $O$ is then:
    ...
    For an arbitrary reaction, we have the sum of reactants and products as:

    \begin{equation}
        \sum_{i=1}^n \nu_i' X_i \leftrightarrows \sum_{i=1}^n \nu_i'' X_i
    \end{equation}
...
\begin{definition}[Reaction rate relations]
    ...
    \begin{equation}
        \ln (K_\text{eq,r}) = A Z^5 + B Z^4 + C Z^3 + D Z^2 + E Z + F 
    \end{equation}
...
\subsection{Example rate equation solution}
    ...
    The forward and backward rate coefficients for the reactions are $k_{f,a}$ and $k_{b,a}$ for the first reaction, all the way down with consistent lettering with the equation number. Recall:

    \begin{equation}
        \frac{d [X_i]}{dt} = (\nu_i'' - \nu_i') \left \{ k_f \prod_i [X_i]^{\nu_i'} - k_b \prod_i [X_i]^{\nu_i''} \right \}
    \end{equation}

    Therefore, filling in and walling the time derivative $\dot{w}_{a \dots j}$, we get: 

    \begin{align}
        $\dot{w}_{a}$ = & (0 - 1) (k_{f,a} [NO][O2] - k_{b,a} [N][O][O_2] ) 
    \end{align}
